...
class UIPreferences(QObject):
    ...
    def _connect(self):
        ...
        self._window.saveFileFilenamePrefixFormat.connect(self.save_file_filename_prefix_format)
        self._window.saveFileOverwriteExistingFile.connect(self.save_file_overwrite_existing_file)
        self._window.saveNetworkSciHubURLs.connect(self.save_network_scihub_urls)
        ...
        self._window.saveNetworkRetryTimes.connect(self.save_network_retry_times)
        self._window.saveNetworkProxyEnabled.connect(self.save_network_proxy_enabled)
        ...
        self.set_file_filename_prefix_format.connect(self._window.setFileFilenamePrefixFormat)
        self.set_file_overwrite_existing_file.connect(self._window.setFileOverwriteExistingFile)
        ...
        self.set_network_proxy_enabled.connect(self._window.setNetworkProxyEnabled)
        ...
        self.set_network_proxy_username.connect(self._window.setNetworkProxyUsername)
        self.set_network_proxy_password.connect(self._window.setNetworkProxyPassword)
    ...
    def load_preferences(self):
        ...
        self.set_system_theme.emit(
            QT_QUICK_CONTROLS2_CONFIG.get_or_default('Material', 'Theme', SYSTEM_THEME_DEFAULT))
        self.set_file_filename_prefix_format.emit(
            Preferences.get_or_default(FILE_FILENAME_PREFIX_FORMAT_KEY, FILE_FILENAME_PREFIX_FORMAT_DEFAULT))
        ...
        self.set_network_scihub_urls.emit(
            Preferences.get_or_default(NETWORK_SCIHUB_URLS_KEY, NETWORK_SCIHUB_URLS_DEFAULT))
        ...
        self.set_network_timeout.emit(
            Preferences.get_or_default(NETWORK_TIMEOUT_KEY, NETWORK_TIMEOUT_DEFAULT, type=int))
        self.set_network_retry_times.emit(
            Preferences.get_or_default(NETWORK_RETRY_TIMES_KEY, NETWORK_RETRY_TIMES_DEFAULT, type=int))
        self.set_network_proxy_enabled.emit(
            Preferences.get_or_default(NETWORK_PROXY_ENABLE_KEY, NETWORK_PROXY_ENABLE_DEFAULT, type=bool))
        self.set_network_proxy_type.emit(
            Preferences.get_or_default(NETWORK_PROXY_TYPE_KEY, NETWORK_PROXY_TYPE_DEFAULT))
        ...
        self.set_network_proxy_port.emit(
            Preferences.get_or_default(NETWORK_PROXY_PORT_KEY, NETWORK_PROXY_PORT_DEFAULT))
        self.set_network_proxy_username.emit(
            Preferences.get_or_default(NETWORK_PROXY_USERNAME_KEY, NETWORK_PROXY_USERNAME_DEFAULT))
        self.set_network_proxy_password.emit(
            Preferences.get_or_default(NETWORK_PROXY_PASSWORD_KEY, NETWORK_PROXY_PASSWORD_DEFAULT))
